...
import {
  ...
  Star, ArrowRight, Users, Award, Clock
} from 'lucide-react';
...
const EdomoticsWebsite = () => {
  ...
  return (
    <>
      {/* Fixed HEADER with proper content containment */}
      <header className="fixed top-0 left-0 right-0 bg-white backdrop-blur-md shadow-xl z-[9999] transition-all duration-300 overflow-hidden border-b border-gray-200">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 flex-shrink-0 min-w-0">
              ...
              <span className="text-xl md:text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent truncate">
                {siteConfig.logo}
              </span>
            </div>
            <nav className="hidden md:flex space-x-4 lg:space-x-8 flex-shrink-0">
              {navigation.map((item) => (
                <button
                  ...
                  className={`text-gray-700 hover:text-blue-600 transition-colors font-medium whitespace-nowrap text-sm lg:text-base ${
            ...
            <button
              ...
              className="md:hidden text-gray-700 hover:text-blue-600 transition-colors flex-shrink-0 p-1"
            >
              {isMenuOpen ? <X size={24} /> : <Menu size={24} />}
            </button>
          </div>
        </div>
        {/* Mobile menu - properly contained within header */}
        {isMenuOpen && (
              ...
              <div className="flex flex-col space-y-2">
                {navigation.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => scrollToSection(item.id)}
                    className="block w-full text-left py-2 px-2 text-gray-700 hover:text-blue-600 transition-colors rounded hover:bg-gray-50"
                  >
                    {item.label}
                  </button>
                ))}
      ...
      {/* Main wrapper starts here: with proper header offset */}
      <div className="min-h-screen bg-white pt-24">
        {/* Hero Section */}
        <section id="home" className="relative min-h-[calc(100vh-6rem)] flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50">
                ...
                <div className="space-y-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">Integrated solutions for automation and audio-video systems with proven track record since 2025.</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-4">
